document.getElementById('billForm').addEventListener('submit', function(event) {
    ...
    let deadlineDate = new Date(currentDate.getFullYear(), currentDate.getMonth(),0.1); // Assuming deadline is 5th of each month
    let penalty = 0; // Initialize penalty as 0
    if (currentDate > deadlineDate) {
      let daysLate = Math.ceil((currentDate - deadlineDate) / (1000 * 60 * 60 * 24));
      penalty = maintenanceCharge * 0.001 * daysLate; // 2% penalty per day for late payment
    }
